...
import { fetchDonationDetailsStart, fetchDonationRequestThreadStart, fetchDonationItemsStart } from '../../common/redux/donation/donation.actions';
import Moment from 'moment';
import HeaderRightOption from '../../components/HeaderRightOption';
import AddReplies from '../../components/AddReplies';
import { Button, Icon } from 'native-base';

const DonationRequestThreadScreen = ({ replies, request,addRequestThread, fetchDonationDetailsStart, fetchDonationRequestThreadStart, fetchDonationItemsStart, navigation, route }) => {
  ...
  }, [fetchDonationDetailsStart, fetchDonationRequestThreadStart, fetchDonationItemsStart, route.params.id])

  navigation.setOptions({
    title: 'Requests',
    headerRight: () => (
      <Button transparent onPress={() => setAddReply(!addReply)} iconLeft><Icon style={Style.defaultColor} name={'add'} /><Text style={[Style.ml1, Style.mr1, Style.defaultColor]}>Add Replies</Text></Button>
    )
  });
  ...
  return (
    <ScrollView>
      {addReply ? <AddReplies requestDonationId={request.DonationRequestOrganization.Id} onValueSubmit={(data) => onchangeModalStatus(data)} /> : null}
...
const mapDispatch = dispatch => ({
  ...
  addRequestThread: (payLoadData) => dispatch({type: 'ADD_REQUEST_THREAD_START', payload: payLoadData}),
  dispatch
});
