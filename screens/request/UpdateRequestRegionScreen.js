...
const UpdateRequestRegionScreen = (props) => {
  ...
  const requestId = orgParam.requestId;
  ...
  const handleRequest = (type) => {
      ...
      dispatch({ type: 'MODIFY_REQUEST_REGIONS', payload: data });
  ...
  return (
    <ScrollView style={[Style.pageContainer]}>
      <View style={Style.mv2}>
        ...
        <Button onPress={() => handleRequest('Volunteer')} block success><Text style={[Style.textWhite, Style.fontSizeNormal]}>Save Region</Text></Button>
...
export default connect(mapState)(UpdateRequestRegionScreen);
